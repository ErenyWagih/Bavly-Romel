...
export default function Contact() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // ✨ تجهيز mailto بروفيشينال
    const subject = encodeURIComponent(`New message from ${name}`);
    const body = encodeURIComponent(`${message}`);
    const mailtoLink = `mailto:Bavlyromel3@gmail.com?subject=${subject}&body=${body}`;

    // فتح الإيميل الافتراضي عند المستخدم
    window.location.href = mailtoLink;

    setSent(true);
    setTimeout(() => setSent(false), 3000); // 3000ms = 3 ثواني
    setName("");
    setEmail("");
    setMessage("");
  };
  ...
  return (
    <motion.section
      id="contact"
      className="min-h-screen px-4 sm:px-6 md:px-20 py-20 sm:py-28 md:py-32 flex items-center justify-center"
